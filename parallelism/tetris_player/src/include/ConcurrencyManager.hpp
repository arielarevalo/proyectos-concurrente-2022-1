...
// TODO(aarevalo): No repetir los errores de nuestros ancestros (memoria)
...
private:
	static const TetrisWorkUnit stopCondition;
};

const TetrisWorkUnit ConcurrencyManager::stopCondition;
...
void ConcurrencyManager::processTetrisState()
{
	...
	auto queue = new Queue<TetrisWorkUnit>();

	auto producer = new TetrisProducer(units, numConsumers);
	...
	{
		auto consumer = new TetrisConsumer(queue, stopCondition,
				false);
