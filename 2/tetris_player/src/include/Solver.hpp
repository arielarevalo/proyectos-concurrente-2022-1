...
private:
	 ...
	 */
	bool findBestMoves(PlayState& current, int currentDepth);
	 ...
	 */
	bool findChildren(PlayState& parent, int parentDepth);
	 ...
	 */
	bool compareToBest(PlayState& current);
...
bool Solver::findBestMoves(PlayState& current, int currentDepth)
{
	...
	if (currentDepth < finalDepth)
	{
		...
		if (currentDepth > INITIAL_DEPTH && isHighScore)
		{
...
bool Solver::findChildren(PlayState& parent, int parentDepth)
{
	bool isHighScore{ false };
...
bool Solver::compareToBest(PlayState& current)
{
	bool isHighScore{ false };
	...
	{
		unsigned int currentScore{ current.getScore() };
		unsigned int topScore{ history.front().getScore() };
